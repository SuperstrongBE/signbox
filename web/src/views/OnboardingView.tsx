/**
 * The ESR onboarding flow reached from the CLI's companion link (URL hash).
 * Extracted verbatim from the original single-purpose App — behavior unchanged.
 * It carries its OWN network in the payload; the global selector does not
 * apply here (styles live in index.css, not the appshell theme).
 */
...
export function OnboardingView({ payload }: { payload: OnboardPayload }) {
  ...
  const authorityMismatch = session !== null && session.actor !== payload.summary.authority;
...
function summarizeActions(payload: OnboardPayload): { name: string; detail: string }[] {
